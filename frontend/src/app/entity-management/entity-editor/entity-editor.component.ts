...
export class EntityEditorComponent implements OnInit {
  ...
  @Input()
  components: Map<number, DesignerComponent>;
  component_list: DesignerComponent[];
  @Output()
  onDestroyEntity = new EventEmitter<any>();
  @Output()
  onNameChange = new EventEmitter<string>();
  @Output()
  onDestroyComponent = new EventEmitter<number>();
  @Output()
  onNewComponent = new EventEmitter<any>();
  ...
  buildComponentList() {
    if (this.entity) {
      this.component_list = [];
      for (const c_id of this.entity.components) {
